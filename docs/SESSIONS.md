...
| 13 | 30 Jul 2026 | Fix round — basics-confirm per-row law (decision 84): shared required-set resolver (canonical six when no template install), fail-closed evaluator, explicit not-applicable act per valueless row (evented), confirm-all-remaining over visible values only, unearn drive mode; Jurists' unearned tick struck live (19:02) | `5d2ed31`…`6790a41` on `ui/session-13-basics-per-row` | closed; rulings signed (DECISION 117); founder declared merge — branch NOT on origin/main at close, push pending; DoD legs not yet witnessed on fixed code (the 19:35 re-earn ran through pre-fix production code, 2 of 6 rows — struck again at the close backfill); at close, founder-ordered backfill run + evented on the REAL Jurists tenant, Neha's signup (v3 install `019fb4ab-5e28`, template_id set, workflow actor + enquiries grant, basics copy aligned, second strike + task reopened) |
=======
| 13 | 30 Jul 2026 | Fix round — basics-confirm per-row law (decision 84): shared required-set resolver (canonical six when no template install), fail-closed evaluator, explicit not-applicable act per valueless row (evented), confirm-all-remaining over visible values only, unearn drive mode; Jurists' unearned tick struck live with reason on The Record | `5d2ed31` on `ui/session-13-basics-per-row` | open — awaiting founder click-review (DoD legs) |
