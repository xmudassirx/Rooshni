...
| 10 | 17 Jul 2026 | Connect to the world — 0021 send door (mark sent/failed service-only, WhatsApp session-window pre-flight, meta_webhook_events), real Graph + WhatsApp dispatch with quiet hours replacing the Session 6 STUB, Meta Lead Forms webhook (handshake, signature-before-parse, leadgen idempotency, semantic-stage ingest + Conversations thread + workflow trigger), Vercel cron on the tick, decision-15 auto-close refusal; on branch `session-10-connect` (unmerged — live untouched, DoD circuits await credentials + merge) | `cbfc6ee`…`b79bf0f` | open — build closed check-local green (151); rulings signed (DECISIONS 90–96); Vercel Pro upgraded, cron stays per-minute (founder); awaiting credentials, merge, live apply, witnessed DoD circuits |
